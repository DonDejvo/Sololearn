...
(async () => {
    ...
    class Challenges {
        ...
        static createChallenge(challenge) {
            ...
            const style = (bg, w = 72) => `padding: 4px; color: white; text-transform: uppercase; font-size: 10px; font-weight: bold; display: inline-block; width: ${w}px; text-align: center; background: ${bg};`;
            let html = `<div class="challenge-body" style="display:flex; justify-content: space-between; gap: 6px; padding: 6px 8px; background-color:#fff; border-radius: 8px; border: none; box-sizing: border-box;">
                <div style='display: flex; gap: 6px; flex-basis: 30%;'>
                ...
                <div style="display: flex; gap: 4px; align-items: center;">
                            <span style="${style("#2493df", 32)}">${courses.find(e => e.id == challenge.courseID).name}</span>
                            ${(() => {
                                switch(challenge.player.status) {
                                    case 1: return `<span style="${style("#0c0")}">You won</span>`;
                                    case 2: return `<span style="${style("#f00")}">You lost</span>`;
                                    case 5: return `<span style="${style("silver")}">Waiting</span>`;
                                    case 6: return `<span style="${style("silver")}">Declined</span>`;
                                    case 3: case 4: return `<div style="display: flex; flex-direction: column; gap: 4px;">
                                    <button data-id="${challenge.id}" class="challenge-accept-btn sol-button sol-button-primary sol-button-block sol-button-s" style="${style("#2493df")}">Accept</button>
                                    <button data-id="${challenge.id}" class="challenge-decline-btn sol-button sol-button-primary sol-button-block sol-button-s" style="${style("#2493df")}">Decline</button>
                                    </div>`;
                                    default: return `<span style="${style("#2493df")}">Draw</span>`;
                                }
                            })()}
                        </div>
                </div>
